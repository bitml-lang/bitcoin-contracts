...
class Test_Alice extends AnyFunSuite with BeforeAndAfterAll with LazyLogging {
  ...
  test("Alice") {
    ...
    log.printStatus("Trying to assemble transaction fundsA")
    var future = contract ? TryAssemble("fundsA", autoPublish = true)
    var res = cm.getResult(future)
    log.printStatus("Assembled fundsA with raw " + res)
    ...
    contract ? Authorize("TwinB")

    while(!published) {
      log.printStatus("Trying to assemble transaction T")
      contract ! AskForSigs("TwinA")
